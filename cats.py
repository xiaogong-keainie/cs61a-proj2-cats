...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    size1 = len(typed_words)
    size2 = len(source_words)
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    check = True
    for s in word_list:
        diff_now = diff_function(typed_word, s,limit)
        if (min_diff >= diff_now and check) or (min_diff > diff_now):
            min_diff = diff_now
            ans = s
            check = False
    return ans
...
def furry_fixes(typed, source, limit):
    ...
    if typed[0] == source[0]:
        return furry_fixes(typed[1 : size1], source[1 : size2], limit)
    else:
        return 1 + furry_fixes(typed[1 : size1], source[1 : size2], limit - 1)
    # END PROBLEM 6
...
def report_progress(typed, source, user_id, upload):
    ...
    dict_pro = {'id':user_id, 'progress':progress}
    upload(dict_pro)
    return progress
    # END PROBLEM 8
...
def fastest_words(match_object):
    ...
    for w in word_indices:
        ...
        for pl in player_indices:
            temp_time = get_time(match_object, pl, w)
            if temp_time < time_min:
                time_min = temp_time
                id_pl_min = pl
        ans[id_pl_min].append(get_word(match_object, w))
    return ans
    # END PROBLEM 11
